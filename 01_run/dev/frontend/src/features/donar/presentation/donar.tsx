...
import type { AcopioBogota } from '@features/evento/domain/entities/evento';
...
const DonarContenido: FC<{ acopioBogota: AcopioBogota }> = ({ acopioBogota }) => {
  const conNecesidades = departamentosUseCases.listarConNecesidades();

  return (
    <>
      <AlertBanner nivel="critico" titulo="Dinero: solo Cruz Roja Colombiana">
...
export const Donar: FC = () => {
  const { data, error, isLoading } = useResumenEvento();
  return (
    <Render
      isLoading={isLoading}
      error={error}
      SuccessComponent={() => (data ? <DonarContenido acopioBogota={data.acopioBogota} /> : null)}
    />
  );
};
